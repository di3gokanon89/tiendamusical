...
/**
 * 
 * @author DevPredator Clase que controla el flujo de la pantalla de
 *         administracion de artistas.
 */
@ManagedBean
@ViewScoped
public class AdminArtistasController {
	...
	/**
	 * Lista que permite filtrar los artistas del datatable
	 */
	...
	@PostConstruct
	public void init() {
		LOGGER.info("Cargando pantalla de administración de artistas...");
		this.consultar();
		this.cargarCombos();
	...
	public void guardar() {
		...
		try {
			...
			CommonUtils.mostrarMensaje(FacesMessage.SEVERITY_INFO, "ERROR!", "Hubo un error al guardar la imagen del artista, favor de intentarlo más tarde o contactar con soporte.");			
			e.printStackTrace();
		}
		...
		if (artistaGuardado.getIdArtista() != null) {	
			...
			CommonUtils.mostrarMensaje(FacesMessage.SEVERITY_INFO, "ERROR!", "Hubo un error al guardar el artista, favor de intentarlo más tarde o contactar con soporte.");			
		}
	 ...
	 * @param artista {@link Artista} objeto con el artista a actualizar.
	 */
